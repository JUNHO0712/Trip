...
| **Headplane UI** | `http://192.168.45.245:3000` |
| Pre-auth Key | 관리자 문의 |
...
또는 직접 다운로드: https://tailscale.com/download/mac
...
- `<AUTH_KEY>`는 관리자가 공유한 key를 이용해주세요
...
└─────────┼────────────────┼────────────────┼─────────────────┘
         ...
         ▼         ▼       ▼       ▼         ▼
      ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐   
      │k8s-cp│ │k8s-w1│ │k8s-w2│ │k8s-w3│   etc
...
- **Headscale 서버 관리자**: kylekim1223@gmail.com
---

## Headplane UI (웹 관리 콘솔)

Headplane은 Headscale의 웹 기반 관리 UI입니다. 노드, 사용자, ACL 정책 등을 시각적으로 관리할 수 있습니다.

### 접속 방법

1. 브라우저에서 `http://192.168.45.245:3000` 접속
2. **API Key로 로그인** 선택
3. Headscale API Key 입력

### API Key 획득

관리자에게 API Key를 요청하거나, 호스트 서버에서 다음 명령어로 생성:
...
| 기능 | 설명 |
|------|------|
| 노드 관리 | 등록된 기기 목록, 상태 확인, 이름 변경, 삭제 |
| Pre-auth Key | 새 기기 등록용 키 생성 및 관리 |
| ACL 정책 | 네트워크 접근 제어 정책 설정 |
| DNS 설정 | MagicDNS 및 커스텀 DNS 레코드 관리 |
| 사용자 관리 | Namespace(사용자 그룹) 생성 및 관리 |
...
│  Headplane Dashboard                                    │
│  ┌──────────────────────────────────────────────────┐  │
│  │  Nodes          Users          ACLs      Keys    │  │
│  ├──────────────────────────────────────────────────┤  │
│  │  k8s-cp        ● online      100.64.0.1          │  │
│  │  k8s-w1        ● online      100.64.0.2          │  │
│  │  k8s-w2        ● online      100.64.0.4          │  │
│  │  k8s-w3        ● online      100.64.0.3          │  │
│  └──────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────┘
```
